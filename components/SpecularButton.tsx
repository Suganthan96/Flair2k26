...
export interface SpecularButtonProps {
  ...
  /** Renders an <a> instead of a <button> when set — needed for anything
   * (e.g. a third-party embed script) that expects real anchor semantics. */
  href?: string;
  /** Arbitrary extra attributes (e.g. data-* for a third-party script) passed
...
const SpecularButton = ({
  ...
  // Runs after every render (no dependency array) rather than during render,
  // so the rAF loop below always reads current props without needing to be
  // torn down and restarted on every prop change.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      // Cast is safe: `steer` already checked `pointerAngle != null` above,
      // TS just can't carry that narrowing through the intermediate boolean.
